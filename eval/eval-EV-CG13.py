...
def valid_argument_type(arg, reftype):
    if arg == "Cause" or re.match(r'^Theme\d*$', arg):
        ...
        return reftype in valid_argument_types
...
op.add_option("-d","--pred_directory",action="store",dest="preddir",metavar="DIRECTORY",default=".",help="the directory where predicted files are placed.")
...
args = [options.preddir + file_name for file_name in os.listdir(options.preddir) if os.path.isfile(os.path.join(options.preddir, file_name)) and file_name.endswith(".a2")]
