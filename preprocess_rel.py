...
def parse_standoff_file(standoff_file, text_file, encoding=None):
    ...
    for line in read_lines(standoff_file, encoding=encoding):
        ...
        if line.startswith(
            ...
            if entity_type.startswith("MMLite"):
                continue
...
def write_ann_file(
    ...
    if modalities:
        for k in sorted(modalities, key=lambda val: int(val.lstrip("MA"))):
            lines.append(
                "{}\t{} {}".format(
                    modalities[k]["id"],
                    modalities[k]["type"],
...
def convert(corpus_name, corpus_dir, output_dir):
    ...
    for fn in glob(os.path.join(corpus_dir, "**/*.txt"), recursive=True):
        ...
        if os.path.exists(a2_file):
            _, _, rels, events, _, _, _ = parse_standoff_file(
                a2_file, fn, encoding="UTF-8"
            )

            for rel in rels.values():
                defined_types["relation_types"].add(rel["role"])
    ...
    for fn in glob(os.path.join(corpus_dir, "**/*.txt"), recursive=True):
        ...
        a2_file = basename + ".ann"
        if os.path.exists(a2_file):
            original_doc, a2_entities, a2_relations, a2_events, a2_modalities, a2_attributes, a2_equivalences = parse_standoff_file(
                a2_file, fn, encoding="UTF-8"
            )
            entities.update(a2_entities)
        ...
        for relation in relations.values():
            relation["role"] = relation["role"].strip(string.digits)

            arg_id = relation["left_arg"]["id"]
            ...
            relation["left_arg"]["id"] = arg_id

            arg_id = relation["right_arg"]["id"]
        ...
        #         if not v["id"].startswith("TR")
        #     },
        #     normalise_triggers=True,
        # )
